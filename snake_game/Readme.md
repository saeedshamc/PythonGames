...
### روش ۱: اجرا مستقیم (بدون نصب)
این ساده‌ترین روش برای اجرای بازی هست.

#### پیش‌نیاز
پایتون باید روی سیستم نصب باشه (نسخه‌ی ۳.۸ به بالا کافیه).

#### نصب کتابخانه‌ی pygame
توی ترمینال یا CMD، این دستور رو بزن:
```
pip install pygame-ce
```
> اگه نسخه‌ی پایتونت خیلی جدید باشه (مثلاً ۳.۱۳ یا ۳.۱۴) و نصب `pygame` معمولی خطای build بده، به‌جاش از `pygame-ce` استفاده کن؛ چون این نسخه wheel آماده داره و کامپایل لازم نداره.
...
### روش ۲: نصب با pip (برای استفاده دائمی)
این روش بازی رو به صورت یک پکیج نصب می‌کنه و می‌تونی از هر جایی اجراش کنی.
...
#### اجرا
بعد از نصب، می‌تونی از هر جایی با این دستور بازی رو اجرا کنی:
```
snake-game
```

#### حذف
اگه خواستی بازی رو حذف کنی:
```
pip uninstall snake-game
```

### روش ۳: نسخه Portable (بدون نیاز به پایتون)
این روش یک فایل اجرایی مستقل (exe) می‌سازه که نیاز به نصب پایتون نداره.

#### ساخت نسخه Portable
۱. اول PyInstaller رو نصب کن:
```
pip install pyinstaller
```

۲. بعد اسکریپت ساخت رو اجرا کن:
```
python build_portable.py
```

۳. فایل اجرایی توی پوشه‌ی `portable` ساخته می‌شه. می‌تونی کل پوشه رو کپی کنی و روی هر کامپیوتری بدون نصب چیزی اجراش کنی.
