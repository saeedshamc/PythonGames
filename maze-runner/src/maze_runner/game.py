...
from .config import (
    ...
    COLOR_PLAYER, COLOR_PLAYER_GLOW, COLOR_GOAL, COLOR_ENEMY,
...
class Game:
    ...
    def load_level(self, level_num):
        ...
        if level_num == 1:
            ...
            w, h, n_enemies, time_limit, enemy_speed = cfg
            ...
            open_cells = [
                (x, y) for y, row in enumerate(grid) for x, v in enumerate(row)
                if v == 0
            ]
            open_cells.sort(
                key=lambda c: -((c[0] - start[0]) ** 2 + (c[1] - start[1]) ** 2)
            )
            candidates = open_cells[:max(1, len(open_cells) // 3)]
            for _ in range(n_enemies):
                ex, ey = random.choice(candidates)
                self.enemies.append(Enemy(ex, ey, enemy_speed))
    ...
    def update(self):
        ...
        for enemy in self.enemies:
            enemy.update(now, self.grid, self.player, self.sound)
            if (enemy.x, enemy.y) == self.player:
                self.on_caught()
                return
    ...
    def draw_maze(self):
        ...
        for enemy in self.enemies:
            ecx = enemy.x * CELL_SIZE + CELL_SIZE // 2
            ecy = self.hud_height + enemy.y * CELL_SIZE + CELL_SIZE // 2
            wob = int(2 * abs((enemy.pulse % 6) - 3))
            pygame.draw.circle(self.screen, COLOR_ENEMY, (ecx, ecy), CELL_SIZE // 2 - 3 + wob)
            pygame.draw.circle(self.screen, (255, 255, 255), (ecx - 4, ecy - 3), 2)
            pygame.draw.circle(self.screen, (255, 255, 255), (ecx + 4, ecy - 3), 2)
    ...
    def draw_hud(self):
        ...
        time_color = COLOR_TEXT_DIM if remaining > 10 else (240, 90, 90)
    ...
    def draw_menu(self):
        ...
        lines = [
            ...
            "Reach the yellow exit and avoid the red monsters",
            "",
    ...
    def draw(self):
        ...
        if self.state == STATE_WIN:
            ...
            self.draw_center_message(
                "Game Over", "R to try again   |   ESC to quit", COLOR_ENEMY)
